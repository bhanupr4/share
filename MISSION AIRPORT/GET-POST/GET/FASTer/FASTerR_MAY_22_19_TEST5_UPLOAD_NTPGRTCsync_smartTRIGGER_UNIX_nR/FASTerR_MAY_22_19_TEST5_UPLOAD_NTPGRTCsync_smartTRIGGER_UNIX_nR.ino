...
/*
  ...
  kial@123
   ...
   venlitead
   AT+HTTPPARA="URL","http://103.233.79.197:8000/ADSmart.asmx/Demo1?DT=0&DEV=V0020&data="
  ...
  TO DO
  SEND COMMAND WHEN GSM OK

  INCREASE BAUD TO 19200
  TEST ALL CHANGES (TEST CHANGE-)
*/
...
char GSMflush() { //serial flush best for 9600//test change-
  uint8_t b;
  Serial.println(F("Flush"));
  uint32_t timer = (millis());
  //  delay(2);
  while ((((millis()) - timer) < 2) && (GSM.available() <= 0));
  timer = (millis());
  while (GSM.available() > 0)
  { b = GSM.read(); //flush serial
    if (b == '>')
...
String SafeReadUpto(String c, int8_t j) { //j-len//new without flush
  j--;
  uint32_t timer = (millis() / 10);
  String st = "";
  while (GSM.available() <= 0)
  { if ((millis() / 10) - timer > 10)//TEST CHANGE-
      return st;
  }

  uint16_t i = 0;
  while (((millis() / 10) - timer) < 2) {//TEST CHANGE-//tested at 15 missing signal//2-DEAD LINE TIME 20MS FOR 9600
    if (GSM.available() > 0) {
      while (GSM.available() > 0) {
        st += (char)GSM.read();
...
String ReadUpto(String c, int8_t j) { //j-len
  ...
  uint32_t timer = (millis() / 10);
  String st = "";
  while (GSM.available() <= 0)//test CHANGE-
  { if ((millis() / 10) - timer > 10)//TEST CHANGE-
      return st;
  }

  uint16_t i = 0;
  while (((millis() / 10) - timer) < 2) {//test change- //tested at 15 missing signal
    if (GSM.available() > 0) {
      while (GSM.available() > 0) {
        st += (char)GSM.read();
        for (int8_t k = j; k >= 0; k--) {
...
uint8_t mod_read_para(uint8_t id, bool table) { //advanced
  wdrst();
...
////////////////////// READ EEPROM ////////////
String readeepstring(uint16_t i) { //i- starting address //--//Serial.println(F("READING EEPROM String"));
  wdt_reset();
  String temp;
...
bool writeeepstring(uint16_t i, uint16_t l, String *temp) {
  wdt_reset();
...
bool GSMprint(String cmd/*, bool ate1*/) { //DEBUG CHANGE-
  //  if (ate1) {
  //    delay(50);
  //    GSM.println(F("ATE1")); wait_ok();
  //  }
  uint32_t t = millis() / 10;
  uint16_t len = lengthof(&cmd);
  //  Serial.println(cmd);
  GSM.println(cmd);
  //    Serial.println(F("sig="));
  //    Serial.println(temp);
  //100% 31 0-31
  st = SafeReadUpto(cmd, len);
  //  Serial.println(st);
  //    Serial.println(temp);
  while (!checkstring(&st, cmd, len)) {
    GSM.println(cmd);
    //    wait_ok();
    st = SafeReadUpto(cmd, len);
    //    Serial.println(st);
    if (checkstring(&st, cmd, len))
      goto sent;

    if ((millis() / 10) - t > 50) //timeout of 300ms
    { Serial.print(F("Command not Sent"));
      Serial.println(10 * ((millis() / 10) - t));
      return false;
    }
  }
  //  delay(200);
  ...
  if (checkstring(&st, cmd, len)) {
sent:
    Serial.print(F("Command sent time "));
    Serial.println(10 * ((millis() / 10) - t));
    return true;
  }
}
...
void restartesp(bool cmd) { //PASS
  ...
  { Serial.println(F("rst GSM"));
    ...
    GSMprint(F("AT+CFUN=1,1"));
  }
  wait_ok();
  //  st = SafeReadUpto(F("RDY"), 3);
  //  st = readGSM();
  //  if (checkstring(&st, F("RDY"), 3))
  //  {
  //    if (checkstring(&st, F("Ready"), 5))
...
void restartesp_bad(bool cmd) { //PASS Serial.println(F("Checking GSM"));
  wdrst();
  if (cmd == true)
  { Serial.println(F("rst GSM"));
    //    GSM.println(F("AT+CPOWD=0"));
    ...
    while (!checkstring(&st, F("RDY"), 3)) {
      ...
      if (checkstring(&st, F("RDY"), 3))
        break;
      if ((millis() / 10) - t > 1000) //timeout of 10000ms
      { return restartesp(1);//force to restart
      }
      delay(2000);
    }
  }
  ...
  else if (checkstring(&st, F("RDY"), 3))
  {
    Serial.println(F("RDY DETECTED"));
    //    return;
  }
  wdrst();
}
...
void set_apn()
{
  ...
  if (checkstring(&st, F("+CSP"), 4))
  {
    ...
    else if (checkstring(&st, F("!dea"), 4))//change--
      apn = F("internet");
...
//GSM NTP
bool NTPconnect(bool check) { //to connect with server (id, check wifi)
  heal_esp();
  Serial.println(F("NTP CONNECT"));
  if (check)
  {
    if (!check_2g())
  ...
  GSMprint(F("AT+CIPSHUT"));//REPLY- SHUT OK
  wait_ok();//wait for busy s.. string
  GSMprint(F("AT+CIPMUX=0"));
  wait_ok();//wait for busy s.. string
  //  GSMprint(F("AT+CIPQSEND=1"));//DATA ACCEPTED
  //  wait_ok();//wait for busy s.. string
  //  GSMprint(F("AT+CIPSPRT=1"));//enable reply of">"
  //  wait_ok();//wait for busy s.. string
  //  GSMprint(F("AT+CIPHEAD=1"));//ENABLE +IPD HEAD
  //  wait_ok();//wait for busy s.. string

  //  GSM.println(F("AT+CIPSTART=\"UDP\",\"india.colorado.edu\",\"123\""));//port always 123
  //  GSM.println(F("AT+CIPSTART=\"UDP\",\"130.102.128.23\",\"123\""));//port always 123
  //  GSM.println(F("AT+CIPSTART=\"UDP\",\"216.239.35.0\",\"123\""));//port always 123

  //BEGIN UDP WITH NTP
  //    GSM.println(F("AT+CIPSTART=\"UDP\",\"in.pool.ntp.org\",\"123""));//port always 123
  //best
  Serial.println(F("UDP"));
  //  delay(500);
  GSMprint(F("AT+CIPSTART=\"UDP\",\"time.google.com\",\"123\""));//port always 123
  st = ReadUpto(F("T O"), 3);
  Serial.println(st);
  uint32_t timer = millis();
  //st =Serial3.readString();//NOT NECESSARY BEC WAIT READ AND STORE DATA
  while (!(checkstring(&st, F("T O"), 3) || checkstring(&st, F("AL"), 2)))
  { wdrst();
    if (checkstring(&st, F("CLO"), 3) || checkstring(&st, F("T F"), 3))//CONNECT FAIL
      goto closed;
...
bool cipsend(uint8_t len)
{ heal_esp();
  //  s = 2;
  //  sending = true;
  Serial.print(F("SEND"));
  GSMprint(F("AT+CIPSEND"));
  //  GSM.println(len);
  st = ReadUpto(F(">"), 1);
  uint32_t timer = millis();
  while (!checkstring(&st, F(">"), 1))
  { wdrst();
...
uint32_t Get_NTP_time_unix()
{
  uint8_t secError;
  secError = millis() / 100;
  GSMprint(F("AT+CIPCLOSE"));
  wait_ok();
  //check 2g
  ...
  if (NTPconnect(1)) //if server not rechable//print to lcd server reach fail and return;
  {
    byte packetBuffer[NTP_PACKET_SIZE];
    memset(packetBuffer, 0, NTP_PACKET_SIZE);//SET ALL BUFFER TO ZERO
    // Initialize values needed to form NTP request

    packetBuffer[0] = 0b11100011;   // LI, Version, Mode//0b11100011;
    packetBuffer[1] = 0;     // Stratum, or type of clock//level of time source
    ...
    if (cipsend(NTP_PACKET_SIZE))
    {
      GSM.write(packetBuffer, NTP_PACKET_SIZE); //(ARR,48)
      //      for (byte i = 0; i < NTP_PACKET_SIZE; i++)
      //      { GSM.write(packetBuffer[i]);
      //        delay(2);
      //      }
      GSM.write(0x1A);//to terminate send
    }
    else
    {
      GSMprint(F("AT+CIPCLOSE"));
    ...
    //    delay(2100);
    st = SafeReadUpto(F("48:"), 3);
    Serial.println(st);
    uint32_t t = millis() / 100;
    while (!checkstring(&st, F("+IP"), 2))
    {
...
got_unix:
  ...
  uint32_t unix = 0;
  //  uint32_t timer = millis() / 10;
  //  while (((millis() / 10) - timer) < 20) {//tested at 15 missing signal
  //    if (GSM.available() > 0) {
  //      while (GSM.available() > 0)
  //      {
  //        byte ch = GSM.read();
  //        if (i >= 16 && i <= 19)//get time data
  //        {
  //          //DATA START AT
  //          //      packetBuffer[i] = ch;
  //          unix = unix << 8;
  ...
  { byte ch = GSM.read();
    byte b = 19;
    if (i >= 16 && i <= b)//get time data FOR WIFI
      //    if (i <= NTP_PACKET_SIZE)
      //    if (i >= 18 && i <= 21)//get time data FOR GSM
    {
      //DATA START AT
      //      packetBuffer[i] = ch;
      unix = unix << 8;
...
out:
  GSMflush();
  Serial.print(i);
  Serial.println(F(" bytes received")); // will be more than 48
  if (i == 0)
    return Get_NTP_time_unix();
  //  GSM.println(F("AT+CIPCLOSE"));
  //  wait_ok();
  secError = ((millis() / 100) - secError);
  //  Serial.println(secError);
  if (secError % 5 != 0)
    secError += 5;
  secError /= 5;
  if (secError >= 2)
    secError /= 2;
  unix += secError; //to overcome internet delay
  //  Serial.println(secError);
  //  Serial.print("SecsSince1900= ");
  //  Serial.print(unix, DEC);

  // Unix time starts on Jan 1 1970. In seconds, that's 2208988800:
  //  const unsigned long seventyYears  = 2208988800UL;
  // subtract seventy years:
  unix -= 2208988800UL;//convert to epoch= - seventy year
  unix += 19800;//indian GMT

  //  Serial.println();
  Serial.print("UNIX: ");
  Serial.println(unix, DEC);
  return unix;
}

String padd0(uint16_t d)
{ String s = "";
  if (d < 10)
  { //padding '0'
    s = F("0");
...
bool sync_RTC()
{ Serial.println(F("Sync NTP Time"));
  uint32_t t = millis() / 100;
  uint32_t now_time;
  now_time = Get_NTP_time_unix();
  while (!sync_CCLK(now_time))
  {
    //      Serial.println(st);
    wdrst();
    ...
    if (((millis() / 100) - t) > 350 && now_time == 0) //15SEC 5000/100=50
...
#define GSM1 GSM
//#define GSM1 GSM// FOR SIM900A
bool sync_CCLK(uint32_t epochCalc)
{
  if (epochCalc == 0)
    return false;
  //1575680235 error
  //For example to set date/time for 3rd March 2018 9 PM GMT
  //  "yy/MM/dd,hh:mm:ss±zz"
  //  AT+CCLK="18/03/03,21:00:00+22"
  //  Serial.println(epochCalc);
  byte second = epochCalc % 60; epochCalc /= 60;
  byte minute = epochCalc % 60; epochCalc /= 60;
  byte hour   = epochCalc % 24; epochCalc /= 24;

  uint8_t month_days[12] = {31, 28, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31};
  //  uint8_t week_days[7] = {4, 5, 6, 0, 1, 2, 3};

  GSM1.print("AT+CCLK=");
  //YEAR
  uint16_t ntp_year = 1970 + (epochCalc / 365); //2019-119
  uint16_t temp = 0;
  for (uint16_t i = 1972/*1972*/; i < ntp_year; i += 4) // Calculating number of leap days since epoch/1970
  { if (((i % 4 == 0) && (i % 100 != 0)) || (i % 400 == 0))
      temp++;//count leap days
  }
  ntp_year = (1970 + ((epochCalc - temp) / 365)); //2019=19 // Calculating accurate current year by (days_since_epoch - extra leap days)
  GSM1.print(F("\""));
  GSM1.print(padd0(ntp_year % 100)); //YEAR //to get 2digit

  //MONTH
  //check current year and update feb
  ...
  ntp_year = ((epochCalc - temp) % 365); //why+1//no of days found in current year
  ntp_year++;
  //  Serial.println();
  //  Serial.println(temp);
  //  Serial.println(epochCalc);
  //  Serial.println(epochCalc - temp);
  //  Serial.println(ntp_year);
  //  //month calc
  temp = 0;
  epochCalc = ntp_year;
  for (uint8_t ntp_month = 1; ntp_month <= 12; ntp_month++) //calculating current Month
  {
    if (temp >= epochCalc)break;
    temp += month_days[ntp_month - 1]; //TEMP_DAYS
    ntp_year = ntp_month;
  }
  GSM1.print(F("/"));
  GSM1.print(padd0(ntp_year));//MONTH
  //Serial.print(ntp_year);//MONTH
  //DATE
  temp -= month_days[ntp_year - 1]; //calculating current Date
  GSM1.print(F("/"));
  ntp_year = epochCalc - temp;
  GSM1.print(padd0(ntp_year)); //DAYS
  GSM1.print(F(","));
  // print the hour, minute and second:
  //HH
  GSM1.print(padd0(hour)); // print the hour (86400 equals secs per day)
  GSM1.print(F(":"));
  //MM
  GSM1.print(padd0(minute)); // print the minute (3600 equals secs per minute)
  GSM1.print(F(":"));
  //SS
  GSM1.print(padd0(second)); // print the second
  GSM1.println(F("+22\""));
  //WAIT FOR OK
...
void setting() { //to set a sever //PASS
  ...
  Serial.println(F("Checking time"));
  if (now_unix() < 1557290878)
  { if (!sync_RTC())
      goto rst;
...
rtcok:
  ...
  GSMprint(F("AT+SAPBR=3,1,\"CONTYPE\",\"GPRS\""));
  wait_ok();
    ...
    { //st[index] = P
      ...
      { //CONNECT GPRS AT+SAPBR=1,1
        ...
        if (!wait_ok()) //wait for busy s.. string
          return false;
        GSMprint(F("AT+SAPBR=1,1"));//CONNECT BARRIER
        //Check for +SAPBR: 1,1
        st = ReadUpto(F("OK"), 2);//CHECK GRPS PROPERLY
        if (!checkstring(&st, F(": 1,1"), 5))
          return false;
        return true;
      }
      return true;
...
uint32_t now_unix()
{
  ...
  wait_ok();
  //    Serial.println(temp);
  while (!checkstring(&st, F("LK: \""), 5)) {
    GSM.println(F("AT+CCLK?"));
    wait_ok();
    //      Serial.println(temp);
    if ((millis() / 10) - t > 200) //timeout of 2000ms
    { return 0;
    }
  }
...
bool upload_data_gateway() { //SET ATE0 THEN SET ATE1//test change-
  //  signaal_led();
  ...
  GSMprint(F("AT+HTTPTERM"));
  if (!wait_ok())
    goto debug;
  //  Serial.println(F("HTTPINIT"));
  GSMprint(F("AT+HTTPINIT"));
  if (!wait_ok())
    goto debug;
  wdrst();
  GSM.println(F("ATE0"));
  ...
  GSM.println(*u.data);//SET ATE0//REQ CHANGE-
  if (!wait_ok())
    goto ate1;//test change-
  GSM.println(F("ATE1"));
  wait_ok();
  /////////////////////////////////////////////////////////////////// copied
  //  Serial.println(F("Closing HTTP"));
  GSMprint(F("AT+HTTPACTION=0"));
  if (wait_ok())
  { if (state == false)
    { Timer1.initialize(50000);
      Timer1.attachInterrupt(UPLOAD_BLINK_WAIT_on);
...
void setup() {
  ...
  Serial.begin(19200);
  Serial.println(freeRam());
  Serial.println(F("F/w: V-0.90FrR"));
  Serial.println(F("F/w: RTC FILTRE&NTPsync"));
...
{ uint8_t j, result;//j- no. of attempts
  ...
  { //get and update j with mod_retry_var
    ...
    Serial.println(freeRam());
    if (mod_read_para(i + 1, 0) != 1) //reading parameter from ram
    { if (mod_read_para(i + 1, 0) == 0) {
        d_mod[i].result = 0; continue;
      }
      else if (mod_read_para(i + 1, 0) == 2) { //ALL read
...
upload://come back after upload
  ...
  *u.data += lu.capture;
  ...
  { //for 16-bit
    ...
    mod_read_para(u_i[i].ix + 1, 0); //had bug solved
...
{ wdrst();
  ...
  { //WHILE1
    ...
    //    ;
    if (((millis() / 100) - modbus_timer) > (30))//everry 3 sec check time
    { //timer
      uint32_t now_time;
check:
      now_time = now_unix(); //check this function
      Serial.println(now_time); //debug change-
      Serial.println(now_time - old_unix); //debug change-
      Serial.println(old_unix); //debug change-
      //check for invalid time ad debud it
      if (now_time == 0 || (now_time < old_unix))//CHANGE+
      { //time problem
        //RESTART MODULE AND SET TIME
        Serial.println(F("Time ERROR"));
        //restart is failed use NTP to get time
        //2nd option increase 1hr
        if (!sync_RTC())
          goto done;//go back to main
        else
        {
          goto check;
        }
