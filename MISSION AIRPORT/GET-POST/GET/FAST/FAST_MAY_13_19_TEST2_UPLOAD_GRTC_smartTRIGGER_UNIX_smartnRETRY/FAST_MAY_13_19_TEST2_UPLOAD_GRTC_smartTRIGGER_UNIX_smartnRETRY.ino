...
int8_t mod_upload_sync(int8_t i)
{ //  Serial.println(sort());
  //If no data read delete all blocks
  if (i == -2)
  { i = sort();
...
upload://come back after upload
  ...
  *u.data += F("\"");
  ...
  while (!upload_data_gateway())
  { //retry one more time use lu.C_len
    if (lu.C_len >= n)//n times retry
      break;
    Serial.print(F("Retrying "));
    Serial.println(lu.C_len + 1);
    lu.C_len++;
  }
  //  if fail retry n time else goto next itration
  //  if fail log data to eeprom
  Serial.println(F("Deleting Data"));
  Serial.println(freeRam());
  delete u.data;//delete string data
  u.data = NULL;
  Serial.println(freeRam());
  Serial.println(i);
  //GO BACK TO LOOP FOR ITERATION UNTILL ALL DATA SEND
    ...
    { //timer
      ...
      if (now_time != 0) {
        ...
        { //check for old time and add new time of required time period
          ...
          { old_unix += (((now_time - old_unix) / (upload * 5)) * (upload * 5));//tested for 300
          }
        ...
        { //get list of failed data from eeprom log
