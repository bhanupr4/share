...
struct mod
{
  ...
  //  uint8_t slave_id;
  //  uint8_t fun;
  int8_t slave = -1; //for status of read
  uint16_t *d;//dynamic
} d_mod[nBlock];
...
recopy:
  ...
  { if (d_mod[i].slave == -1)
    ...
    else if (d_mod[i].slave != 0)
    { //check for max and swap if smallest found
      j++;
      if (d_mod[u_i[0].ix].slave < d_mod[i].slave)
      { u_i[j].ix = u_i[0].ix;
        u_i[0].ix = i;
      }
      else if (j != 0)
      { if (d_mod[u_i[j - 1].ix].slave < d_mod[i].slave) //in case of future -1 it false
        { u_i[j].ix = u_i[j - 1].ix;
          u_i[j - 1].ix = i;
        }
        else
...
{ int8_t len = sync();
  ...
  { if (d_mod[u_i[temp].ix].slave != d_mod[u_i[temp + 1].ix].slave)
    { if ((d_mod[u_i[temp].ix].slave - d_mod[u_i[temp + 1].ix].slave) != 1)
      { temp++;
        goto sort;
      }
    }
...
sort:
  ...
  { for (uint8_t j = len; j > i; j--)
    { if (d_mod[u_i[i].ix].slave < d_mod[u_i[j].ix].slave)
      { //swap
        temp = u_i[j].ix;
        u_i[j].ix = u_i[i].ix;
        u_i[i].ix = temp;
...
{ // put your setup code here, to run once:
  ...
  { while (!checkstring(&st, F("TZV:"), 4))//CHECK FOR +HTTPACTION
    {
      //      Serial.println(st);
      wdrst();
      st = Serial2.readString();
      ...
      { Serial.println(F("+CTZV tOUT"));
        goto clts;
      }
    }
    Serial.println(F("RTC"));
    if (!checkstring(&st, F("TZV:"), 4))
    { //SET HARDWARE AND RESTART
clts:
      //      Serial.println(F("AT+CLTS=1"));
      Serial2.println(F("AT+CLTS=1"));
      wait_ok();
...
{ uint8_t j, result;//j- no. of attempts
  ...
  { //get and update j with mod_retry_var
    ...
    { if (mod_read_para(i + 1) == 0) {
        d_mod[i].slave = 0; continue;
      }
      else if (mod_read_para(i + 1) == 2) { //ALL read
        d_mod[i].slave = -1; return;
      }
    }
    else
    { //delete old
...
retry:
      ...
      { //mod success
        ...
        d_mod[i].slave = mod.slave;//for sorting as slave id
...
{ //  Serial.println(sort());
  ...
  { i = sort();
    ...
    { Serial.print(i);
      ...
      Serial.print(F("\t-->\t"));
      Serial.println(d_mod[u_i[i].ix].slave);
      i--;
    }
    i = sort();
    i++;//tested Ok
...
upload://come back after upload
  ...
  while ((lu.send_len <= lu.safe_send) && (i >= 0)) //and restrict different slave id //pivot
  { i--;
    if (i == -1) {
      i++; break;
    }
    //    Serial.println(F("Grouping"));
    mod_read_para(u_i[i].ix + 1);
    lu.send_len += (10 + (4 * mod.registers));
    Serial.print(F("Grp i ="));  Serial.println(i);    Serial.print(F("send len ="));    Serial.println(lu.send_len);
    if (i > 0)
      if (d_mod[u_i[i - 1].ix].slave != d_mod[u_i[i].ix].slave)
        break;
  }
  ...
  *u.data += lu.capture;
  ...
  { //for 16-bit
    ...
    *u.data += h2s(&lu.temp, 8); //length =2
    //    lu.temp = (String*)calloc(4, 1);
    for (lu.C_len = 0; lu.C_len < mod.registers; lu.C_len++)
  ...
  *u.data += F("\"");
  ...
  //  while (i >= 0)
  //  { send_len += (10 + (4 * mod_data[u_i[i].ix].slave));
