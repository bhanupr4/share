...
#define ADD 1//EEPROM START ADDRESS
...
volatile int32_t count = 0, BUZZtout = 0;
...
volatile int8_t STATE[size_STATE] = {0, 0, 0, 0, 0}, i = -1; //Record footprint
...
void buzzON() {
  ...
  else if (BUZZtout > 0 && BUZZtout <= Buzz) {
    BUZZtout++;
    //    Serial.println(BUZZtout);
  }
  else {
...
void STATE_COUNT() {
  ...
  //3-(L,L)-0+1=1
  //4-(H,L)-2+1=3
  volatile uint8_t temp_state = digitalRead(S2);
  temp_state |= digitalRead(S1) << 1;
  temp_state++;
  //Serial.println(temp_state);
  /*if (((i + 1) < size_STATE)&& ()) {
    //record footprint
    if (STATE[i] != temp_state) {
      //record footprint
      i++;
      STATE[i] = temp_state;
    }
    ...
    //check footprint and reset
    for (uint8_t j = 0; j < size_STATE; j++) {
      Serial.print(',');
  ...
  //  ((count * len) >= (BUZZth * 100)) ? buzzON() : buzzOFF();//buzzing if crossed over limits

  volatile uint32_t BUZZcount = ((BUZZth * 100) / len);//in cm
  //check for reminder in BUZZth
  //if (((BUZZth * 100.00) > len) ) //if reminder
  //  (count > BUZZcount && count < BUZZcount) ? buzzON() : buzzOFF(); //buzzing if crossed over limits
  //else
  //  (BUZZcount == count) ? buzzON() : buzzOFF(); //buzzing if crossed over limits

  (count == BUZZcount) ? buzzON() : buzzOFF(); //buzzing if crossed over limits

  //  if ((count <= BUZZcount) && ((count + 1) >= BUZZcount))
  //    buzzON();
  //  else
  //    buzzOFF();
  ...
  if (temp_state != 4) {
    ...
    //    }
    if (STATE[1] == 1) {
      if ((STATE[0] != STATE[2]) && (STATE[3] == 0) && (STATE[4] == 0)) {
        if (STATE[0] == 2) {
          //++
          count++;
...
{ pinMode(RESET, INPUT_PULLUP);
  ...
  Timer1.initialize(1000);//(1000);//(10000);
  Timer1.attachInterrupt(STATE_COUNT);
  timer = millis() / 100;
